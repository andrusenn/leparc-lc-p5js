...
if (!p5.prototype.hasOwnProperty("mediaPath")) {
    p5.prototype.mediaPath = function () {
        ...
        if (arg.length == 0) {
            return Lp5.main
                .path()
                .join(Lp5.main.resourcesPath(), "leparc_resources", "media");
        }
        if (arg.length == 1) {
            return Lp5.main
                .path()
...
if (!p5.prototype.hasOwnProperty("libsPath")) {
    p5.prototype.libsPath = function () {
        ...
        if (arg.length == 0) {
            return Lp5.main
                .path()
                .join(Lp5.main.resourcesPath(), "leparc_resources", "libs");
        }
        if (arg.length == 1) {
            return Lp5.main
                .path()
    ...
    p5.prototype.audioBeat = function (band) {
        if (Lp5.mode == "SERVER" || Lp5.mode == "LOCAL") {
            if (___fft != null && ___audio != null) {
                ...
                console_msg(
                    "audioBeat() -> useAudio() " + lang_msg.not_declared,
                );
                return null;
            }
        } else {
    ...
    p5.prototype.audioEnergy = function (f1, f2 = null) {
        if (Lp5.mode == "SERVER" || Lp5.mode == "LOCAL") {
            if (___fft != null && ___audio != null) {
                ...
                console_msg(
                    "audioEnergy() -> useAudio() " + lang_msg.not_declared,
                );
                return null;
            }
        } else {
            console_msg(
                "audioEnergy() " + lang_msg.local_server_mode,
                "warning",
            );
        }
    };
...
if (!p5.prototype.hasOwnProperty("tpulse")) {
    p5.prototype.tpulse = function (_t = 500, _g = 100, _of = 0) {
        ...
        if (_of < 0) _of = 0;
        if (
            Math.floor((((millis() + _of) / _t) * 2) % _t) % 2 == 0 &&
            Math.floor(millis() + _of) % _t < _g
        ) {
            return true;
        } else {
            return false;
...
if (!p5.prototype.hasOwnProperty("useOSC")) {
    p5.prototype.useOSC = function () {
        ...
        if (
            arg.length == 1 &&
            (typeof arg[0] === "string" || arg[0] instanceof String)
        ) {
            ip = arg[0];
        }
...
if (!p5.prototype.hasOwnProperty("useMIDI")) {
    p5.prototype.useMIDI = function () {
        ...
        if (arg.length === 2) {
            ch = arg[0];
            if (typeof arg[1] === "string") deviceName = arg[1];
            if (typeof arg[1] === "number") deviceIndex = arg[1];
        }

        if (deviceIndex === null && deviceName === null) {
            // All
            Lp5.midiInputs.forEach((input) => {
                Inputs(input, ch);
            });
...
// BUFFERS --- PGRAPHICS -> 6
if (!p5.prototype.hasOwnProperty("___BUFFER0")) {
    ...
    p5.prototype.useBuffer = function (n, r = P2D) {
        for (let i = 0; i < Lp5.numBuffers; i++) {
            if (n === i) {
                ...
                ___BUFFER[i] = createGraphics(width, height, r);
                ___BUFFER[i].elt.setAttribute("id", "bufferId" + i);
                ___BUFFER[i].elt.dataset.render = r;
            }
        }
    };
}
        ...
        try {
            for (let i = 0; i < Lp5.numBuffers; i++) {
                if (arg[0] === i) {
                    push();
                    imageMode(CORNER);
                    image(___BUFFER[i], x, y);
                    pop();

                    return ___BUFFER[i].clear();
                }
            }
        } catch (e) {
            console.log(e);
